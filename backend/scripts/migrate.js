...
async function seedSelfCardDomain(connection) {
  const configuredBase = String(
    process.env.PUBLIC_CARD_BASE_URL || 'https://vod.zzqixiangkeji.cn',
  ).trim();
  let url;
  try {
    url = new URL(configuredBase);
...
async function disableLegacyTestDomains(connection) {
  // Keep historical short links for auditability, but prevent retired test
  // domains from being selected for newly generated links.
  await connection.execute(
    `UPDATE domains
     SET is_enabled = 0,
         is_primary = 0
  ...
  const [[activePrimary]] = await connection.execute(
    `SELECT id
     FROM domains
  ...
  // A retired primary must be replaced so domain selection never falls into
  // an empty-primary state. Prefer the formal public origin, then any enabled
  // self-hosted domain as a deterministic fallback.
  const preferredDomains = [
    process.env.PUBLIC_CARD_BASE_URL,
    process.env.PUBLIC_SHORTLINK_BASE_URL,
    'https://vod.zzqixiangkeji.cn',
  ]
  ...
  for (const domain of preferredDomains) {
    const [[row]] = await connection.execute(
      `SELECT id
       FROM domains
       WHERE domain = ? AND platform = 'self' AND is_enabled = 1
       LIMIT 1`,
      [domain],
    );
    if (row) {
  ...
  if (!candidate) {
    const [[row]] = await connection.execute(
      `SELECT id
       FROM domains
       WHERE platform = 'self' AND is_enabled = 1
       ORDER BY id ASC
       LIMIT 1`,
    );
    candidate = row;
  ...
  await connection.query('UPDATE domains SET is_primary = 0');
  await connection.execute('UPDATE domains SET is_primary = 1 WHERE id = ?', [candidate.id]);
  await connection.execute(
    `INSERT INTO system_configs (config_key, config_value)
...
async function migrate() {
  ...
  try {
    ...
    await disableLegacyTestDomains(connection);
