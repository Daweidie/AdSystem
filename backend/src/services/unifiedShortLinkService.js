...
function reconcileConfiguredSuolinkDomain(domains, config = {}) {
  ...
  // suolink_enabled = 1 时保留所有已启用的 Suolink 域名，生成器会按
  // 已分配链接数在域名池中均衡选择；配置域名仅用于并列时的优先级。
...
async function getAvailableDomains() {
  const [configRows, domainResult] = await Promise.all([
    ...
    `SELECT d.id, d.domain, d.type, d.platform, d.is_primary,
            ...
            COALESCE(usage_count.last_link_id, 0) AS last_link_id
     FROM domains d
     LEFT JOIN (
       SELECT domain_id, COUNT(*) AS link_count, MAX(id) AS last_link_id
       FROM short_links
       GROUP BY domain_id
     ) usage_count ON usage_count.domain_id = d.id
     WHERE d.is_enabled = 1
       ORDER BY COALESCE(usage_count.last_link_id, 0) ASC,
                COALESCE(usage_count.link_count, 0) ASC,
              d.is_primary DESC,
              d.id ASC`,
    ),
  ]);
...
function selectCandidates(domains, requestedPlatform) {
  ...
  const balanced = (items) => [...items].sort((left, right) =>
    Number(left.last_link_id || 0) - Number(right.last_link_id || 0) ||
...
// 生成时返回同平台的完整候选列表。createShortLink 会从最少使用的域名
// 开始尝试并在成功后停止；下一次生成会因 link_count 增加而自然轮换到
// 其他启用域名。失败域名仍会继续尝试，避免单个供应商域名故障阻断生成。
function selectGenerationCandidates(domains, requestedPlatform) {
  return selectCandidates(domains, requestedPlatform);
}
...
/**
 * 短链统一入口。auto 模式先按使用量尝试启用的 Suolink 域名池，
 * 全部失败后再按 fallback 规则尝试可用的自建域名。
 *
...
async function createShortLink(longUrl, platform = 'auto', options = {}) {
  ...
  if (options.preferredSelfOrigin && options.requirePreferredSelfOrigin
    ...
    if (preferredOrigin && options.requirePreferredSelfOrigin) {
      const actualSelfDomains = generationDomains
        .filter((domain) => domain.platform === 'self')
        .map((domain) => domain.domain);
      generationDomains = generationDomains.filter((domain) => {
        if (domain.platform !== 'self') return true;
        try {
      ...
      if (requestedPlatform === 'self'
        && !generationDomains.some((domain) => domain.platform === 'self')) {
        const actual = actualSelfDomains.length
          ? actualSelfDomains.join('、')
          : '（无启用的 self 域名）';
        throw new UnifiedShortLinkServiceError(
          `公共卡片域名 ${preferredOrigin} 与启用的 self 域名不一致；实际域名：${actual}。`
            + '请将期望域名加入并启用为 self_hosted/self，或修正 PUBLIC_CARD_BASE_URL。',
          'PUBLIC_CARD_DOMAIN_MISMATCH',
          409,
        );
      }
    }
  ...
  // 管理台显式传 allowFallback=false 时只禁止跨平台降级，仍会继续尝试
  // 同平台域名池中的其他候选；兼容调用未关闭 fallback 时，主服务明确
  // 失败后再启用同平台备用域名或 auto 自建容灾。
  ...
  for (const domain of [...candidates, ...fallbackCandidates]) {
    ...
    try {
      ...
      providerResult = await suolinkService.createShortLink(longUrl, {
        ...
        // Suolink 供应商按北京时间（UTC+8）解释 expireDate；直接对 UTC 日期
        // 切片会在 UTC 与北京时间交替的边界（UTC 16:00-24:00）把过期日算成
        // 前一天，供应商会立即把短链判为无效。这里按 +8 小时取供应商所在
        // 时区的日期，与项目业务时区（北京时间）保持一致。
        expireDate: new Date(
          new Date(video.expires_at).getTime() + 8 * 60 * 60 * 1000,
