...
async function saveSuolinkConfig(req, res, next) {
  ...
  try {
    ...
    if (req.body.enabled) {
      // 配置域名作为首选域名参与并列排序；域名池中其他已启用的
      // Suolink 域名也会参与新链接生成并按使用量均衡轮换。
...
async function getCardDomainConfig(req, res, next) {
  void req;
  try {
    res.json({ success: true, data: await getPublicCardDomainDiagnostics() });
  } catch (error) {
    next(error);
  }
}
