...
Write-Host ''
Write-Host 'Demo18 corrected deployment' -ForegroundColor Cyan
Write-Host 'Enter the ubuntu password once to upload the corrected script.'
Write-Host 'Enter it again for deployment, and again only if sudo asks.'
Write-Host 'Typed password characters will not be displayed or logged.'
Write-Host ''

& scp.exe @sshOptions $remoteScriptPath 'ubuntu@49.232.124.39:/tmp/'
if ($LASTEXITCODE -ne 0) {
  Set-Content -LiteralPath $failurePath -Value "scp exit code: $LASTEXITCODE" -Encoding ascii
  exit $LASTEXITCODE
}

& ssh.exe -tt @sshOptions 'ubuntu@49.232.124.39' 'sudo bash /tmp/demo18-remote-deploy.sh' 2>&1 |
