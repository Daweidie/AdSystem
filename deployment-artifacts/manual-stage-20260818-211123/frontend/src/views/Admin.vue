...
const myVisitQuotaBaseInput = ref(0);
const myVisitQuotaAddition = ref('');
const myVisitQuotaActionLoading = ref(false);
...
const canManageVisitQuota = computed(() =>
  ['super_admin', 'system_admin', 'business_manager'].includes(user.value?.role),
);
...
const menus = computed(() => {
  ...
  if (isPlatformAdmin.value && !isSuper.value) {
    sections.push({ title: '系统设置', icon: '☆', items: [{ key: 'visit-quotas', label: '访问量管理' }] });
  }
  if (isSuper.value) {
    sections.push({
      title: '系统设置', icon: '☆',
...
async function loadMyVisitQuota() {
  ...
  try {
    ...
    myVisitQuotaBaseInput.value = Number(myVisitQuota.value.baseQuota || 0);
  } catch (error) {
...
async function addMyVisitQuota() {
  const amount = Number(myVisitQuotaAddition.value);
  if (!Number.isInteger(amount) || amount < 1 || amount > 100000000) {
    ElMessage.warning('本月额外额度必须是 1 到 100000000 之间的整数');
    return;
  }
  myVisitQuotaActionLoading.value = true;
  try {
    await request('/management/visit-quota/add', {
      method: 'POST',
      body: JSON.stringify({ additionalQuota: amount }),
    });
    myVisitQuotaAddition.value = '';
    ElMessage.success(`本月额外额度已立即增加 ${amount} 次`);
    await loadMyVisitQuota();
  } catch (error) { ElMessage.error(error.message); }
  finally { myVisitQuotaActionLoading.value = false; }
}
async function updateMyVisitQuotaBase() {
  const baseQuota = Number(myVisitQuotaBaseInput.value);
  if (!Number.isInteger(baseQuota) || baseQuota < 1 || baseQuota > 100000000) {
    ElMessage.warning('本月基础额度必须是 1 到 100000000 之间的整数');
    return;
  }
  myVisitQuotaActionLoading.value = true;
  try {
    await request('/management/visit-quota/base', {
      method: 'PUT',
      body: JSON.stringify({ baseQuota }),
    });
    ElMessage.success(`本业务组本月基础额度已立即更新为 ${baseQuota} 次`);
    await loadMyVisitQuota();
  } catch (error) { ElMessage.error(error.message); }
  finally { myVisitQuotaActionLoading.value = false; }
}
...
async function loadDomainPool() {
  const [domainList, suolinkConfig] = await Promise.all([
    request('/domain/list'), request('/domain/suolink-config'),
  ]);
...
function validatedWechatShareUrl(link) {
  ...
  if (link.platform === 'self') {
    if (url.hostname !== 'vod.hotwharf.com' || !/^\/s\/[A-Za-z0-9]{6,8}$/.test(url.pathname)) {
      throw new Error('自建链接二维码只能使用 https://vod.hotwharf.com/s/{shortCode}');
    }
      ...
      <main class="admin-content" v-loading="loading">
        ...
        <template v-else-if="activePage === 'materials'">
          ...
          <section v-if="canViewVisitQuota && myVisitQuota" class="business-quota-summary" aria-label="本月访问额度">
            ...
            <div v-if="user.role === 'business_manager'" class="business-quota-actions">
              <label><span>修改本业务组本月基础额度（立即生效）</span><el-input-number v-model="myVisitQuotaBaseInput" :min="1" :max="100000000" :step="100" controls-position="right" /></label>
              <el-button type="primary" :loading="myVisitQuotaActionLoading" @click="updateMyVisitQuotaBase">保存本月基础额度</el-button>
              <label><span>增加本月额外访问额度（立即生效）</span><el-input-number v-model="myVisitQuotaAddition" :min="1" :max="100000000" :step="100" controls-position="right" placeholder="次数" /></label>
              <el-button type="primary" plain :loading="myVisitQuotaActionLoading" @click="addMyVisitQuota">增加额度</el-button>
            </div>
          </section>
  ...
  <div class="upload-zones"><label class="upload-zone"><input type="file" accept="video/mp4,video/*" @change="pickFile($event, 'video')" /><strong>▶</strong><b>{{ uploadForm.videoFile?.name || '选择视频文件' }}</b><span>建议 MP4，文件不超过 800MB</span></label><label class="upload-zone cover-zone"><input type="file" accept="image/jpeg,image/png,image/webp" @change="pickFile($event, 'cover')" /><strong>＋</strong><b>{{ uploadForm.coverFile?.name || '上传封面图片' }}</b><span>支持 JPG、PNG、WebP，文件不超过 5MB</span></label></div>
        ...
        <template v-else-if="['promoters', 'admins', 'expiring', 'business-groups'].includes(activePage)">
          ...
          <div class="data-table-wrap">
            <table v-if="activePage === 'business-groups'" class="data-table"><thead><tr><th>序号</th><th>业务组名称</th><th>管理员</th><th>登录手机</th><th>成员数</th><th>添加时间</th><th>到期时间</th><th>状态</th><th>操作</th></tr></thead><tbody><tr v-for="(item, i) in businessGroups" :key="item.id"><td>{{ i + 1 }}</td><td><b>{{ item.name }}</b></td><td>{{ item.manager_name || '-' }}</td><td>{{ item.manager_phone || '-' }}</td><td>{{ item.member_count }}</td><td>{{ formatDate(item.created_at) }}</td><td>{{ formatDate(item.expires_at) }}</td><td><span class="table-status" :class="{ disabled: item.status !== 'active' }">{{ item.status === 'active' ? '正常' : '停用' }}</span></td><td class="table-actions"><button type="button" @click="editBusinessGroup(item)">编辑</button></td></tr></tbody></table>
        ...
        <template v-else-if="activePage === 'customer-link'">
          <div class="domain-pool-card">
            <div class="link-config-heading">
              ...
              <div><h2>客户域名池</h2><p>集中维护客户访问域名；同一平台的新推广链接固定使用一个域名。</p></div>
            ...
            <section class="suolink-config-card">
              ...
              <p class="suolink-config-tip">保存后的 Suolink Domain 是新链接的唯一生成域名，并必须已绑定到当前 API Key。修改域名只影响之后生成的链接，历史链接保持原地址；API Key 将加密保存且不会回显明文。</p>
            </section>
            ...
            <div class="domain-pool-table-wrap">
              <table class="data-table domain-pool-table">
                <thead><tr><th>域名</th><th>备注</th><th>已分配链接</th><th>状态</th><th>角色</th><th>操作</th></tr></thead>
                <tbody><tr v-for="item in domains" :key="item.id">
                  <td><code>{{ item.domain }}</code></td><td>{{ item.remark || '-' }}</td><td>{{ item.link_count || 0 }}</td>
            ...
            <div class="link-config-note"><b>生成规则</b><p>同一平台的新推广链接只使用一个固定域名，不在池内域名之间轮换。切换主域名或 Suolink 配置只影响之后生成的链接，历史链接保持原地址。</p><p>主域名同时作为播放地址兜底。微信正式投放建议使用已备案并配置 HTTPS 的域名。</p></div>
          </div>
        </template>
        ...
        <template v-else-if="activePage === 'visit-quotas'">
          <div class="visit-quota-page">
            ...
            <div class="visit-quota-table-wrap">
              <table class="data-table visit-quota-table">
                <thead><tr><th>业务组</th><th>有效推广员</th><th>本月基础额度</th><th>追加额度</th><th>本月总额度</th><th>已使用</th><th>剩余</th><th>修改本月基础额度</th><th>增加本月额外额度</th></tr></thead>
                <tbody>
                  <tr v-for="group in visitQuotaData.groups" :key="group.businessGroupId">
                    <td><b>{{ group.businessGroupName }}</b></td>
                    <td>{{ group.effectiveEmployees }}</td>
                    <td>{{ group.baseQuota.toLocaleString() }}</td>
                    <td>{{ group.extraQuota.toLocaleString() }}</td>
                    <td>{{ (group.baseQuota + group.extraQuota).toLocaleString() }}</td>
                    <td>{{ group.usedQuota.toLocaleString() }}</td>
                    <td><strong :class="{ 'quota-empty': group.remainingQuota === 0 }">{{ group.remainingQuota.toLocaleString() }}</strong></td>
                    <td class="visit-quota-add-cell">
                      <el-input-number v-model="visitQuotaBases[group.businessGroupId]" :min="1" :max="100000000" :step="100" controls-position="right" />
                      <el-button type="primary" plain @click="updateVisitQuotaBase(group)">保存本月基础额度</el-button>
                    </td>
                    <td class="visit-quota-add-cell">
                      <el-input-number v-model="visitQuotaAdditions[group.businessGroupId]" :min="1" :max="100000000" :step="100" controls-position="right" placeholder="次数" />
                      <el-button type="primary" plain @click="addVisitQuota(group)">增加</el-button>
                    </td>
                  </tr>
                </tbody>
              </table>
              <el-empty v-if="!visitQuotaData.groups.length" description="暂无业务组" />
            </div>
    ...
    <el-dialog v-model="wechatShareVisible" title="微信分享" width="min(620px, 92vw)" destroy-on-close @closed="resetWechatShareDialog">
      <div class="wechat-share-dialog">
        <div class="wechat-share-details">
          <span class="link-platform-tag" :class="wechatShareForm.platform">{{ wechatShareForm.platform === 'suolink' ? 'Suolink' : '自建' }}</span>
        ...
        <div class="wechat-share-qr">
          ...
          <p v-if="wechatShareForm.platform === 'suolink'" class="suolink-share-note">若需要分享原 Suolink 短域名，请使用 Suolink 后台提供的分享功能；播放页只能恢复同源 /card 地址。</p>
        </div>
      </div>
    </el-dialog>
