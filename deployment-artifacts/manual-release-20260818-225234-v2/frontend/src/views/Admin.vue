...
async function selectPage(key) {
  if (key === 'visit-quotas' && !isPlatformAdmin.value) {
    ElMessage.error('只有平台管理员可以管理访问量');
    return;
  }
...
async function updateMyVisitQuotaBase() {
  ...
  try {
    ...
    ElMessage.success(`本月基础额度已立即更新为 ${baseQuota} 次`);
    await loadMyVisitQuota();
  } catch (error) { ElMessage.error(error.message); }
  finally { myVisitQuotaActionLoading.value = false; }
}
      ...
      <main class="admin-content" v-loading="loading">
        ...
        <template v-else-if="activePage === 'materials'">
          ...
          <section v-if="canViewVisitQuota && myVisitQuota" class="business-quota-summary" aria-label="本月访问额度">
            ...
            <div v-if="canManageVisitQuota && !isPlatformAdmin" class="business-quota-actions">
              <label><span>增加本月额外额度（立即生效）</span><el-input-number v-model="myVisitQuotaAddition" :min="1" :max="100000000" :step="100" controls-position="right" placeholder="次数" /></label>
              <el-button type="primary" :loading="myVisitQuotaActionLoading" @click="addMyVisitQuota">增加额度</el-button>
              <label><span>修改本月基础额度（立即生效）</span><el-input-number v-model="myVisitQuotaBaseInput" :min="1" :max="100000000" :step="100" controls-position="right" /></label>
              <el-button type="primary" plain :loading="myVisitQuotaActionLoading" @click="updateMyVisitQuotaBase">保存基础额度</el-button>
            </div>
          </section>
  ...
  <div class="upload-zones"><label class="upload-zone"><input type="file" accept="video/mp4,video/*" @change="pickFile($event, 'video')" /><strong>▶</strong><b>{{ uploadForm.videoFile?.name || '选择视频文件' }}</b><span>建议 MP4，文件不超过 800MB</span></label><label class="upload-zone cover-zone"><input type="file" accept="image/jpeg,image/png,image/webp" @change="pickFile($event, 'cover')" /><strong>＋</strong><b>{{ uploadForm.coverFile?.name || '上传封面图片' }}</b><span>支持 JPG、PNG、WebP，文件不超过 5MB</span></label></div>
        ...
        <template v-else-if="activePage === 'customer-link'">
          <div class="domain-pool-card">
            ...
            <div class="link-config-note"><b>生成规则</b><p>Suolink 新推广链接会在已启用供应商域名中优先选择已分配链接数较少的域名；生成失败时会继续尝试其他候选。自建短链使用统一公共卡片入口。切换主域名或 Suolink 配置只影响之后生成的链接，历史链接保持原地址。</p><p>主域名同时作为播放地址兜底。微信正式投放建议使用已备案并配置 HTTPS 的域名。</p></div>
          </div>
        </template>
